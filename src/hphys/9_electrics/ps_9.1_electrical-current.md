...
1. See definitions in textbook.

2. a) $6~\Omega$  
   b) Current doubles if voltage doubles (Ohm's law, $I \propto V$ for constant $R$)

3. a) $0.09~\mathrm{A}$  
   b) $0.045~\mathrm{A}$ (current halves)

4. $40~\Omega$

5. a) $0.3~\mathrm{V}$  
   b) LEDs are polarized; current must flow in the correct direction

6. $6~\Omega$ each (series) or $24~\Omega$ each (parallel, if answer key expects this)
