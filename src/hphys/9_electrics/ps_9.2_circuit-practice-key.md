...
### 2. Circuit with Single Resistor
...
Three resistors with values $6.00~\Omega $, $12.0~\Omega $, and $18.0~\Omega $ are connected in series with a $36.0~\text{V} $ battery. 
...
<div class="calculations">
 $V_1 = 1.00~\text{A} \times 6.00~\Omega = 6.00~\text{V} $<br>
...
### 4. Christmas Lights in Series

In a string of 50 identical Christmas lights connected in series, each bulb has a resistance of $24~\Omega $. If the string is connected to a $120~\text{V} $ outlet:
...
Three resistors with values $15.0~\Omega $, $30.0~\Omega $, and $45.0~\Omega $ are connected in parallel with a $12.0~\text{V} $ battery.
   (a) Calculate the total resistance of the circuit.
...
Verification: $I_{total} = I_1 + I_2 + I_3$<br> $ 0.800~\text{A} + 0.400~\text{A} + 0.267~\text{A} = 1.47~\text{A} $
</div>
<div class="sigfig">
...
(b) Maximum power the circuit can handle:<br>
 $P_{max} = V \times I_{max} = 120~\text{V} \times 20~\text{A} =$<br> $ 2400~\text{W} $<br>

Available power for additional appliance:<br>
 $P_{available} = P_{max} - P_{heater} $<br> $2400~\text{W} - 1500~\text{W} = 900~\text{W} $
</div>
<div class="sigfig">
...
$R_{60W} = \frac{V^2}{P} = \frac{(120~\text{V})^2}{60~\text{W}} = 240~\Omega $<br>
...
> In series, both bulbs will have the same current. Since $P = I^2R $ and the 60W bulb has higher resistance, it will dissipate more power and be brighter than the 100W bulb (contrary to their ratings at 120V).<br>
